...
public class DatabaseTest {
    ...
    public void testDropDatabase(TestContext context) throws InterruptedException {
        ...
        client.open(Callback.callback(context.asyncAssertSuccess($ -> {
            ...
            db.dropDatabase(Callback.callback(context.asyncAssertSuccess(result -> {
    ...
    public void testDropDatabase2(TestContext context) throws InterruptedException {
        ...
        client.open(Callback.callback(context.asyncAssertSuccess($ -> {
            ...
            db.dropDatabase(Callback.callback(context.asyncAssertSuccess(result -> {
                db.createCollection("testcollection", Callback.callback(context.asyncAssertSuccess(result2 -> {
                    db.getClient().getDatabaseNames(Callback.callback(context.asyncAssertSuccess(list -> {
                        context.assertTrue(list.contains("testdatabase1"));
                        db.dropDatabase(Callback.callback(context.asyncAssertSuccess(result3 -> {
                            db.getClient().getDatabaseNames(Callback.callback(context.asyncAssertSuccess(list2 -> {
